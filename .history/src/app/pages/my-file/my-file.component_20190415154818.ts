...
export class MyFileComponent implements OnInit {
  // pdfSrc: string = 'file:///C:/Users/MandariN/Desktop/pdf-test.pdf';
  ...
  refeshMainFolder(id: number) {
      ...
      }, (err: Response) => { // กรณี token หมดอายุ ให้กลับสู่หน้า login
          console.log('ssssss' + err.message);
  ...
  onDownload() {
    for (const id of this.listChooseCheckBox) {
      this.serviceFile.downloadFile(id).subscribe(
        data => {
          //let pdf = window.URL.createObjectURL(data);
          //console.log(pdf);  //
