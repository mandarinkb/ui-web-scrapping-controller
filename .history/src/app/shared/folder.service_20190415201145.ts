...
export class FolderService {
  ...
  postMainFolder(formData: Folder, id: number) {
   return this.http.post(this.rootURL + '/folder/' + id + '?token=' + this.authen.getAuthenticated(), formData, this.httpOptions);
  }
  ...
  /*this.http
      ...
      */
      return this.http.get(this.rootURL + '/mainfolder-file/' + id + '?token=' + this.authen.getAuthenticated());
  }

  // แก้ไขโฟเดอร์ (ใช้ได้ทั้ง mainfolder และ subfolder)
  editFolder(id: number) {
    return this.http.get(this.rootURL + '/folder/' + id + '?token=' + this.authen.getAuthenticated());
  }
