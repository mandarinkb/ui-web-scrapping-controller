...
import { ShareFileMemberActivity } from './share-file-member-activity.model';
...
export class ShareFileMemberService {
  ...
  listDepartmentActivity: ShareFileMemberActivity[];
  token: string;
  updUser: string;
  memberId: string;
  ...
  // เรียกดูกิจกรรมสมาชิกของหน่วยงาน
  readDepartmentActivity(departmentId: number) {
    return this.http.get(this.rootURL + '/department-activity/' + departmentId + this.token);
  }
  ...
  downloadFile(id: number) {
   ...
   createSubFolder(formSubFolder: Subfolder) {
     return this.http.post(this.rootURL + '/share-file-subfolder/' + this.memberId + this.token, formSubFolder, this.httpOptions);
   }
